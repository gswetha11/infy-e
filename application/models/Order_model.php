...
class Order_model extends CI_Model
{
    ...
    public function consignment_view($order_id = NUll, $seller_id = NULL, $delivery_boy_id = NULL)
    {
        ...
        foreach ($consignment_list as $row) {
            ...
            if (!isset($delivery_boy_id)) {
                $tempRow['otp'] = $row['otp'];
                $operate = '<button type="button" class="btn btn-dark btn-xs mx-1 view_consignment_items" data-toggle="modal" data-target="#view_consignment_items_modal" data-items=\'' . json_encode($item_detail) . '\' data-id=' . $row['id'] . '><i class="fa fa-eye"></i></button>';
            ...
            if ($this->ion_auth->is_seller() || $this->ion_auth->is_admin()) {
                $operate .= '<button type="button" class="btn btn-success mx-1 btn-xs 8596" data-id="' . $row['id'] . '" data-consignment-name="' . $row['name'] . '" data-status="' . $row['status'] . '" data-items=\'' . json_encode($item_detail) . '\' data-toggle="modal" data-target="#consignment_status_modal"><i class="fa fa-pen"></i></button>';
